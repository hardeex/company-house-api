...
MAX_RETRIES = 5
RM_POSTCODES = [f"RM{i}" for i in range(1, 21)]  # All RM postcodes (RM1–RM20)
LOCATIONS = ["Romford", "Dagenham"]  # Add more if needed (e.g., "Havering", "Upminster")
MAX_PAGES = 50  # Fetch up to 5000 companies per location

def fetch_companies(location, cursor=None, retries=0):
    """Fetch companies from Companies House API for a given location."""
    ...
    try:
        ...
        if "total_results" not in data:
            logging.warning(f"total_results missing in response for {location}: {data}")
        logging.info(f"Received {len(data.get('items', []))} companies for {location}, total hits: {data.get('total_results', 'N/A')}")
        return data
        ...
        logging.error(f"HTTP error for {location}: {e} - {response.text}")
        return None
    except requests.exceptions.RequestException as e:
        logging.error(f"Request failed for {location}: {e}")
        return None
...
def extract_and_filter_data(companies):
    ...
    logging.info(f"Filtered {len(filtered)} companies with RM postcodes, overdue accounts, and ltd type")
    return filtered
...
def main():
    """Main function to fetch and process company data for multiple locations."""
    all_data = []
    for location in LOCATIONS:
        ...
        while cursor and page_count < MAX_PAGES:
            ...
            if not result:
                logging.error(f"Failed to fetch data for {location}. Moving to next location.")
                break
