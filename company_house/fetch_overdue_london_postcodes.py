...
MAX_RETRIES = 5
LONDON_POSTCODES = ["SE", "SW", "W", "NW", "NE", "E1", "E2", "E3"]  # Starting postcodes
LOCATIONS = [
...
def check_overdue_accounts(company_number):
    """Check if a company has overdue accounts between April 2025 and August 2025."""
    ...
    try:
        ...
        if next_due:
            ...
            if start_date <= due_date <= end_date and due_date < datetime.now():
                logging.info(f"Overdue accounts detected for {company_number}: next_due={next_due}")
                return True
            else:
                logging.debug(f"No overdue accounts in date range for {company_number}: next_due={next_due}")
        else:
        ...
        for filing in filings:
            if filing.get("category") == "accounts":
                ...
                if filing_date:
                    ...
                    if start_date <= filing_date <= end_date and filing.get("description", "").lower().find("late") > -1:
                        logging.info(f"Overdue accounts detected in filing history for {company_number}")
                        return True
        logging.debug(f"No late filings in history for {company_number} in date range")
        return False
    except requests.exceptions.RequestException as e:
...
def extract_and_filter_data(companies, include_all_types=True):
    """Extract relevant fields and filter by London postcodes, overdue accounts, and company type."""
    filtered = []
    ...
    logging.info(f"Filtered {len(filtered)} companies with London postcodes, overdue accounts, and {'all' if include_all_types else 'ltd'} type")
    return filtered

def save_to_csv(data, filename="overdue_companies_london_apr_aug_2025.csv"):
    ...
    postcodes = df["postcode"].str.extract(r"^(SE|SW|W|NW|NE|E[1-3]\d?)").dropna()[0].unique()
    logging.info(f"Found London postcodes: {', '.join(postcodes)}")
    df.to_csv(filename, index=False)
    logging.info(f"Saved {len(df)} companies to {filename}")

def main():
    """Main function to fetch and process London postcode companies with overdue accounts."""
    all_data = []
