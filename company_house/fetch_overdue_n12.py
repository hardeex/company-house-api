...
MAX_RETRIES = 5
N12_POSTCODES = ["N12", "N12 8"]  # Prioritize N12 8, include all N12
LOCATIONS = ["North Finchley", "Barnet", "London"]  # More specific to broader locations
...
def extract_and_filter_data(companies, include_all_types=True):
    """Extract relevant fields and filter by N12 postcodes and overdue accounts."""
    filtered = []
    ...
    for company in companies:
        ...
        # Check if postcode matches N12 or specifically N12 8
        if not postcode or not any(
            postcode.startswith(pc.replace(" ", "")) for pc in N12_POSTCODES
        ):
            continue
    ...
    logging.info(
        f"Filtered {len(filtered)} companies with N12 postcodes and overdue accounts"
    )
    return filtered
...
def save_to_csv(data, timestamp):
    ...
    if not data:
        ...
        return
    filename = f"overdue_companies_n12_full_{timestamp.strftime('%Y-%m-%d')}.csv"
    df = pd.DataFrame(data)
    df = df.drop_duplicates(subset="company_number")  # Deduplicate
    postcodes = df["postcode"].str.extract(r"^(N12\s?\d?)").dropna()[0].unique()
    logging.info(f"Found N12 postcodes: {', '.join(postcodes)}")
    df.to_csv(filename, index=False)
    logging.info(f"Saved {len(df)} companies to {filename}")


def main():
    """Main function to fetch and process companies with overdue accounts in N12 postcodes."""
    all_data = []
    ...
    save_to_csv(all_data, datetime.now())
