...
def check_overdue_accounts(company_number):
    """Check if a company has accounts overdue by July 31, 2025."""
    ...
    try:
        ...
        if next_due:
            ...
            if due_date <= JULY_END:
                logging.info(
                    f"Accounts due by July 31, 2025 for {company_number}: next_due={next_due}"
                )
                return True, next_due
            else:
                logging.debug(
                    f"No accounts due by July 31, 2025 for {company_number}: next_due={next_due}"
                )
        else:
...
def extract_and_filter_data(companies, include_all_types=False):
    ...
    logging.info(
        f"Filtered {len(filtered)} companies with IG postcodes, accounts due by July 31, 2025, and {'ltd' if not include_all_types else 'all'} type"
    )
    return filtered


def save_to_csv(data, filename="overdue_companies_ig_july_2025.csv"):
