...
LONDON_POSTCODES = [
    ...
    "E2",
    "E3",
]  # Add more E postcodes if needed
LOCATIONS = [
    "Southwark",  # SE
    "Westminster",  # SW, W
    "Camden",  # NW
...
def check_overdue_accounts(company_number):
    """Check if a company has overdue accounts, returning next_due for filtering."""
    ...
    # Fallback: Check filing history for late filings
        ...
        for filing in filings:
            if filing.get("category") == "accounts":
                ...
                if filing_date:
                    ...
                    if filing.get("description", "").lower().find("late") > -1:
                        logging.info(
                            f"Overdue accounts detected in filing history for {company_number}: filing_date={filing_date}"
                        )
                        return True, filing_date
...
def extract_and_filter_data(companies, include_all_types=False):
    """Extract relevant fields and filter by specified postcodes, overdue accounts, and company type."""
    filtered = []
    ...
    for company in companies:
        ...
        if postcode and any(postcode.startswith(pc) for pc in LONDON_POSTCODES):
            logging.info(
                f"Found matching postcode: {company.get('company_name', 'N/A')} ({postcode})"
            )
        else:
            logging.debug(
                f"Skipping {company.get('company_name', 'N/A')}: Postcode {postcode} not in {LONDON_POSTCODES}"
            )
            continue

        company_number = company.get("company_number", "")
        is_overdue, due_date = check_overdue_accounts(company_number)
        if not is_overdue:
            logging.debug(
                f"Skipping {company.get('company_name', 'N/A')}: No overdue accounts"
            )
            continue

        # Filter by date range: April 1, 2025 - July 28, 2025
        start_date = datetime(2025, 4, 1)
        end_date = datetime(2025, 7, 28)
        if due_date and start_date <= due_date <= end_date:
            logging.info(
                f"Company {company.get('company_name', 'N/A')} meets date range criteria: due_date={due_date}"
            )
        else:
            logging.debug(
                f"Skipping {company.get('company_name', 'N/A')}: Due date {due_date} outside April 1-July 28, 2025"
            )
            continue
        ...
        filtered.append(
            {
                ...
                "next_due": due_date.strftime("%Y-%m-%d") if due_date else "N/A",
            }
        )
    logging.info(
        f"Filtered {len(filtered)} companies with specified postcodes, overdue accounts in April 1-July 28, 2025, and {'all' if include_all_types else 'ltd'} type"
    )
    return filtered


def save_to_csv(data, filename="overdue_companies_london_april_july_2025.csv"):
    ...
    postcodes = df["postcode"].str.extract(r"^(SE|SW|W|NW|NE|E\d)").dropna()[0].unique()
    logging.info(f"Found postcodes: {', '.join(postcodes)}")
    df.to_csv(filename, index=False)
    logging.info(f"Saved {len(df)} companies to {filename}")


def main():
    """Main function to fetch and process companies in specified postcodes with overdue accounts in April-July 2025."""
    all_data = []
    include_all_types = True  # Set to False for only ltd companies

    for location in LOCATIONS:
        logging.info(f"Processing location: {location}")
        cursor = "*"
        page_count = 0
        total_results = float("inf")  # Initialize to ensure full pagination
        while cursor and page_count * 100 < total_results:
            ...
            total_results = result.get("total_results", len(companies))
