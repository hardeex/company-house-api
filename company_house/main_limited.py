...
def extract_and_filter_data(companies, include_all_types=False):
    ...
    logging.info(
        f"Filtered {len(filtered)} companies with RM postcodes, overdue accounts, and {'ltd' if not include_all_types else 'all'} type"
    )
    return filtered
...
def save_to_csv(data, filename="overdue_companies_limited_rm.csv"):
    if not data:
        logging.warning("No data to save.")
        return
    df = pd.DataFrame(data)
