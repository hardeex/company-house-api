...
def check_overdue_accounts(company_number):
    """Check if a company has accounts overdue by August 31, 2025."""
    ...
    try:
        ...
        if detailed_status == "active-proposal-to-strike-off":
            logging.debug(
                f"Skipping {company_number}: Status is active-proposal-to-strike-off"
            )
            return False, None
        ...
        if next_due:
            ...
            if due_date <= AUGUST_END:
                logging.info(
                    f"Accounts due by August 31, 2025 for {company_number}: next_due={next_due}"
                )
                return True, next_due
            else:
                logging.debug(
                    f"No accounts due by August 31, 2025 for {company_number}: next_due={next_due}"
                )
        else:
    ...
    # Fallback: Check filing history (less reliable for future dates, so primarily rely on next_due)
...
def extract_and_filter_data(companies, include_all_types=False):
    ...
    for company in companies:
        ...
        if postcode and any(postcode.startswith(ig) for ig in IG_POSTCODES):
            logging.info(
                f"Found IG postcode match: {company.get('company_name', 'N/A')} ({postcode})"
            )
    ...
    logging.info(
        f"Filtered {len(filtered)} companies with IG postcodes, accounts due by August 31, 2025, and {'ltd' if not include_all_types else 'all'} type"
    )
    return filtered


def save_to_csv(data, filename="overdue_companies_ig_august_2025.csv"):
