...
def extract_and_filter_data(companies, postcode_prefix, include_all_types=True):
    ...
    filtered = []
    logging.info(f"Processing {len(companies)} companies for postcode {postcode_prefix}")
    for company in companies:
        ...
        if not postcode or not postcode.startswith(postcode_prefix.replace(" ", "")):
            logging.debug(f"Skipping {company.get('company_name', 'N/A')}: Postcode {postcode} does not match {postcode_prefix}")
            continue

        company_number = company.get("company_number", "")
        has_overdue, next_due = check_overdue_accounts(company_number)
        if not has_overdue:
            logging.debug(f"Skipping {company.get('company_name', 'N/A')}: No overdue accounts")
            continue
        ...
        logging.debug(f"Added {company.get('company_name', 'N/A')} to filtered list for {postcode_prefix}")
    logging.info(f"Filtered {len(filtered)} companies with {postcode_prefix} postcodes and overdue accounts")
    return filtered

def save_to_csv(data, postcode_prefix, timestamp, consolidated_data=None):
    """Save extracted data to CSV with timestamped filename and append to consolidated data."""
    if not data:
        logging.warning(f"No data to save for {postcode_prefix}.")
        return consolidated_data
    filename = f"overdue_companies_{postcode_prefix.lower()}_full_{timestamp.strftime('%Y-%m-%d')}.csv"
    df = pd.DataFrame(data)
    df = df.drop_duplicates(subset="company_number")  # Deduplicate
    postcodes = df["postcode"].str.extract(rf"^({postcode_prefix}\s?\d?)").dropna()[0].unique()
...
def main():
    ...
    consolidated_data = []  # For all N postcodes
    ...
    for postcode, locations in POSTCODE_LOCATIONS.items():
        ...
        for location in locations:
            ...
            while cursor:
                ...
                filtered = extract_and_filter_data(companies, postcode, include_all_types=True)
        ...
        consolidated_data = save_to_csv(all_data, postcode, timestamp, consolidated_data)

    # Save consolidated CSV
    if consolidated_data:
        filename = f"overdue_companies_n_all_full_{timestamp.strftime('%Y-%m-%d')}.csv"
        df = pd.DataFrame(consolidated_data)
        df = df.drop_duplicates(subset="company_number")  # Deduplicate
        df.to_csv(filename, index=False)
        logging.info(f"Saved {len(df)} companies to consolidated {filename}")
